...
def predict_C(
    ...
    """
    x = dataset.x
    pos_scores = model(x, dataset.edge_index, pos)
    neg_scores = model(x, dataset.edge_index, neg.view(P * K, 2)).view(P, K)

    # Add dot-product similarity as extra signal
    def dot_sim(pairs):
        return (x[pairs[:, 0]] * x[pairs[:, 1]]).sum(dim=1)

    alpha = 2.0
    pos_scores = pos_scores + alpha * dot_sim(pos)
    neg_scores = neg_scores + alpha * dot_sim(neg.view(P * K, 2)).view(P, K)
    ################################################
